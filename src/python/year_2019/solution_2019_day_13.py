"""Care Package -- Advent of Code 2019 Day 13."""
...
def run_program(program: List[int]) -> List[int]:
    """Execute the arcade program and collect all outputs.

    Args:
        program (List[int]): Intcode program.

    Returns:
        List[int]: Output values from the machine.
    """
...
def solve_part_one(program: List[int]) -> int:
    """Count block tiles after running the program.
...
def solve_part_two(program: List[int]) -> int:
    """Play the arcade game automatically and return the score.

    Args:
        program (List[int]): Intcode program.

    Returns:
        int: Final score after all blocks are cleared.
    """
    ...
    while not computer.halted:
        out = computer.run(until_output=True, default_input=joystick)
        if not out:
            continue
