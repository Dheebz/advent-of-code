"""Hot Springs -- Advent of Code 2023 Day 12."""
...
def parse_input(raw: str) -> List[Record]:
    """Return list of (pattern, groups)."""
    records: List[Record] = []
    for line in raw.strip().splitlines():
        pattern, groups_str = line.split()
        groups = tuple(int(x) for x in groups_str.split(","))
        records.append((pattern, groups))
    return records


def count_arrangements(pattern: str, groups: Tuple[int, ...]) -> int:
    """Count valid arrangements for a single pattern."""

    @lru_cache(maxsize=None)
    def helper(i: int, gi: int, run: int) -> int:
        if i == len(pattern):
            if gi == len(groups) and run == 0:
                return 1
            if gi == len(groups) - 1 and run == groups[-1]:
                return 1
            return 0

        total = 0
        ch = pattern[i]
        for opt in (".", "#") if ch == "?" else (ch,):
            if opt == "#":
                total += helper(i + 1, gi, run + 1)
            else:
                if run == 0:
                    total += helper(i + 1, gi, 0)
                elif gi < len(groups) and run == groups[gi]:
                    total += helper(i + 1, gi + 1, 0)
        return total
...
def solve_part_one(records: List[Record]) -> int:
    """Total arrangements without expansion."""
    return sum(count_arrangements(pattern, groups) for pattern, groups in records)


def solve_part_two(records: List[Record]) -> int:
    """Total arrangements with fivefold expansion."""
    total = 0
