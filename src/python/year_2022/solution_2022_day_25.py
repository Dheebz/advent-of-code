"""Full of Hot Air -- Advent of Code 2022 Day 25."""
...
def parse_input(raw: str) -> List[str]:
    """Return list of SNAFU numbers."""
    return [line.strip() for line in raw.strip().splitlines() if line.strip()]


def snafu_to_int(s: str) -> int:
    """Convert SNAFU to integer."""
    value = 0
    for ch in s:
        value = value * 5 + VALUES[ch]
    return value
...
def solve_part_one(numbers: List[str]) -> str:
    """Sum numbers and return in SNAFU form."""
    total = sum(snafu_to_int(n) for n in numbers)
