"""Rain Risk -- Advent of Code 2020 Day 12."""
...
def rotate(x: int, y: int, degrees: int) -> Tuple[int, int]:
    """Rotate point around origin clockwise by degrees."""
    turns = (degrees // 90) % 4
...
def solve_part_one(instructions: List[Instruction]) -> int:
    """Manhattan distance after following ship navigation rules."""
    x = 0
    y = 0
    heading = 0  # 0 east, 90 south, 180 west, 270 north (clockwise)
...
def solve_part_two(instructions: List[Instruction]) -> int:
    """Manhattan distance using waypoint navigation."""
    ship_x = 0
    ship_y = 0
    wp_x = 10
    wp_y = 1
