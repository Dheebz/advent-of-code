"""Boiling Boulders -- Advent of Code 2022 Day 18."""
...
def parse_input(raw: str) -> Set[Cube]:
    """Return set of cube coordinates."""
    cubes = set()
    for line in raw.strip().splitlines():
        x_str, y_str, z_str = line.strip().split(",")
        cubes.add((int(x_str), int(y_str), int(z_str)))
    return cubes
...
def exterior_surface(cubes: Set[Cube]) -> int:
    """Count faces reachable from outside using flood fill."""
...
def solve_part_one(cubes: Iterable[Cube]) -> int:
    """Total exposed surface area."""
    return surface_area(set(cubes))


def solve_part_two(cubes: Iterable[Cube]) -> int:
    """Exterior surface area ignoring trapped air pockets."""
    return exterior_surface(set(cubes))
