...
def solve_part_one(n: int) -> int:
    """Manhattan distance from square n to center."""
    if n == 1:
        return 0
    layer = 0
    while (2 * layer + 1) ** 2 < n:
    ...
    side_len = 2 * layer + 1
    max_val = side_len**2
    steps_from_corner = (max_val - n) % (side_len - 1)
    return layer + abs(steps_from_corner - layer)


def solve_part_two(limit: int) -> int:
    """First value written that is larger than the input."""
    grid: Dict[Point, int] = {(0, 0): 1}
    x, y = 0, 0
    ...
    while True:
        for _ in range(2):
            ...
            for _ in range(step_size):
                x += dx
                y += dy
                value = sum(grid.get((x + nx, y + ny), 0) for nx in (-1, 0, 1) for ny in (-1, 0, 1))
                grid[(x, y)] = value
                if value > limit:
                    return value
