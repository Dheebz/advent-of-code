        ...
        for ch in line:
            if ch in OPEN_TO_CLOSE:
                ...
                if not stack or OPEN_TO_CLOSE[stack.pop()] != ch:
                    total += ERROR_SCORES[ch]
                    break
    return total


def solve_part_two(lines: List[str]) -> int:
    """Middle completion score for incomplete lines."""
    scores: List[int] = []
    for line in lines:
        stack: List[str] = []
        corrupted = False
        ...
        score = 0
        while stack:
            closing = OPEN_TO_CLOSE[stack.pop()]
            score = score * 5 + COMPLETE_SCORES[closing]
        scores.append(score)
    scores.sort()
