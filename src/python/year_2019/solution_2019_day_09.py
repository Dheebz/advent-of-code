...
def run_boost(program: List[int], input_value: int) -> int:
    """Run the BOOST program with a given input.

    Args:
        program (List[int]): Intcode program.
        input_value (int): Diagnostic code to feed.

    Returns:
        int: Last output produced by the program.
    """
    computer = Intcode(program)
    outputs = computer.run(inputs=[input_value])
    return outputs[-1]


def solve_part_one(program: List[int]) -> int:
    """Return BOOST keycode for part one."""
    return run_boost(program, 1)


def solve_part_two(program: List[int]) -> int:
    """Return BOOST coordinate for part two."""
    return run_boost(program, 2)
