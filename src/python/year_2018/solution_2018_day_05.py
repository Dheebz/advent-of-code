...
def parse_input(raw: str) -> str:
    """Return the polymer string."""
    return raw.strip()


def react(polymer: str) -> str:
    """Fully react a polymer string."""
    stack = []
    for char in polymer:
...
def solve_part_one(polymer: str) -> int:
    """Length after fully reacting."""
    return len(react(polymer))


def solve_part_two(polymer: str) -> int:
    """Shortest possible length after removing one unit type."""
    types = set(polymer.lower())
    best = len(polymer)
    for t in types:
        reduced = polymer.replace(t, "").replace(t.upper(), "")
        best = min(best, len(react(reduced)))
    return best
