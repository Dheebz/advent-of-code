"""Gear Ratios -- Advent of Code 2023 Day 3."""
...
def solve_part_two(grid: Grid) -> int:
    """Sum of gear ratios for gears touching exactly two numbers."""
    rows, cols = len(grid), len(grid[0])
            ...
            for cc in range(start, c):
                for nr, nc in neighbors(r, cc):
                    if 0 <= nr < rows and 0 <= nc < cols and grid[nr][nc] == "*":
                        gears.add((nr, nc))
            for gear in gears:
                gear_map.setdefault(gear, []).append(value)
    ratio_sum = 0
