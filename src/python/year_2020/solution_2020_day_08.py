"""Handheld Halting -- Advent of Code 2020 Day 8."""
...
def run(program: List[Instruction]) -> Tuple[bool, int]:
    """Run the program, returning (terminated, accumulator)."""
    acc = 0
    ip = 0
    visited = set()
    ...
    return ip == len(program), acc


def solve_part_one(program: List[Instruction]) -> int:
    """Accumulator value immediately before an instruction repeats."""
    _, acc = run(program)
    return acc


def solve_part_two(program: List[Instruction]) -> int:
    """Accumulator after fixing the program by swapping a single jmp/nop."""
